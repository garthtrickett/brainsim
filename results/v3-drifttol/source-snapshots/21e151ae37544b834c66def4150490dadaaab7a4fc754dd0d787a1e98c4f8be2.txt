"""Boundary-perturbation equations, tolerance authority and full reproduction."""
import argparse
import ast
import copy
import hashlib
...
from report_v3_drifttol import absolute_summary, publish_report, summarize
from study_v3_drifttol import (BOUNDS, LEVELS, SOURCES, confirm, make_manifest, measure_row,
    perturbed_bounds, read_stage, registration_check, require_committed, schedule_rng,
    simulation, source_hashes, true_bounds, tune, verified_manifest, write)
from v3_adwin import run as adwin_run
from v3_forgetting import run as forget_run
from v3_drifttol import ARMS, ESTIMATOR, FAMILIES, FAST, GRIDS, run
from v3_drifttol_policy import (KEYS, PROTOCOL, SEEDS, SPECS, complete, contrast,
    digest, metric_values, objective, performance_decision, scientific, select, tolerance_point)
...
def kernel_checks():
    ...
    assert set(LEVELS) == {'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50'}
    assert BOUNDS == {'jitter_32': 32, 'jitter_256': 256, 'jitter_1024': 1024}
    assert len(SPECS) == 15 and [len(KEYS[s]) for s in ('tuning', 'confirmation')] == [288, 288]
    assert len(ARMS) == 9
    ...
    assert all(min(s) >= 170000 and max(s) < 176000 for s in parts)
    ...
    values = np.random.default_rng(170001).normal(size=600)
    ...
    # Boundary perturbation: abrupt times exact, drift moves bounded, order kept.
    y = np.random.default_rng(170000).normal(size=(500, 2))
    for name in FIXTURES:
        data = fixture(170001, name)
        order, width = FIXTURES.index(name), len(data.coordinates)
        assert np.array_equal(true_bounds(data), true_marks(data))
        drifts = [(t, j) for j, ev in enumerate(data.events) for t, k in ev if k in ('drift_start', 'drift_end')]
        for arm in ('jitter_32', 'jitter_256', 'jitter_1024'):
            schedule, enter = perturbed_bounds(data, arm, 170001, order)
            assert schedule.shape == data.y.shape and set(np.unique(schedule)) <= {0., 1.}
            assert set(np.unique(enter)) <= {0., 1.} and (enter <= schedule).all()
            for j, events in enumerate(data.events):
                for start, kind in events:
                    if kind.startswith('target_'):
                        assert schedule[start, j] == 1.
            for t, j in drifts:
                window = schedule[max(0, t-BOUNDS[arm]):t+BOUNDS[arm]+1, j]
                assert window.sum() >= 1.
            assert np.array_equal(schedule, perturbed_bounds(data, arm, 170001, order)[0])
            if drifts:
                assert not np.array_equal(schedule, perturbed_bounds(data, arm, 170002, order)[0])
        kept, kept_enter = perturbed_bounds(data, 'drop_50', 170001, order)
        assert ((kept <= true_marks(data)).all() and (kept_enter <= kept).all())
        assert np.array_equal(kept, perturbed_bounds(data, 'drop_50', 170001, order)[0])
    # Order enforcement on a synthetic inverted pair.
    from types import SimpleNamespace
    narrow = SimpleNamespace(y=np.zeros((6000, 1)), events=[[(3000, 'drift_start'), (100, 'drift_end')]])
    schedule, enter = perturbed_bounds(narrow, 'jitter_1024', 170001, 2)
    indices = sorted(np.flatnonzero(schedule[:, 0]).tolist())
    assert len(indices) == 2 and indices[1] == indices[0]+1
    assert sorted(np.flatnonzero(enter[:, 0]).tolist()) == [indices[0]]
    # Frozen parity: scheduled arms are the slope dual-state at ESTIMATOR.
    triggers = np.zeros_like(y); triggers[200] = 1.
    for a, b in zip(run(y, 'reference', dict(ESTIMATOR), schedule=triggers, enter=np.zeros_like(triggers)),
                     __import__('v3_slope').run(y, 'slope', dict(ESTIMATOR), schedule=triggers, enter=np.zeros_like(triggers))):
    ...
    for arm in ARMS:
        config = GRIDS[arm][3] if arm in FAMILIES else dict(ESTIMATOR) if arm != 'oracle_nofallback' else dict(FAST)
        if arm in FAMILIES:
            kwargs = {}
        elif arm == 'oracle_nofallback':
            kwargs = {'schedule': triggers}
        else:
            schedule, enter = perturbed_bounds(proxy, arm, 170000, 0) if arm in LEVELS else (triggers, np.zeros_like(triggers))
            kwargs = {'schedule': schedule, 'enter': enter}
    ...
    rejects(lambda: run(y, 'reference', {'method': 'ols', 'W': 64}, schedule=triggers, enter=np.zeros_like(triggers)))
    rejects(lambda: run(np.full_like(y, np.nan), 'window', {'window': 16}))
    rejects(lambda: run(y, 'reference', dict(ESTIMATOR)))
    for name in FIXTURES:
        with patch('study_v3_drifttol.perturbed_bounds', side_effect=AssertionError('evaluator access')):
            with patch('study_v3_drifttol.true_bounds', side_effect=AssertionError('evaluator access')):
                for arm in FAMILIES:
                    simulation(170001, name, arm, GRIDS[arm][3])
    print('PASS drifttol kernels: boundary perturbation, frozen parity, causality, privilege isolation', flush=True)
...
def synthetic():
    ...
    def adjust(r, arm):
        ...
        if arm in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50'):
            for f in r['memory'].values():
                for c in f.values():
    ...
    for a in ARMS:
        for seed in SEEDS['confirmation']:
            ...
            if a in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50'):
...
def policy_checks():
    ...
    assert objective(tuning['window/0/171000']) == 1.
    assert all(i == 0 for i in manifest['screen']['indices'].values())
    assert manifest['screen']['selected']['oracle_nofallback'] == dict(FAST)
    assert all(manifest['screen']['selected'][a] == dict(ESTIMATOR) for a in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50'))
    assert tolerance_point(rows)['tolerance'] == 1024
    assert performance_decision(rows, 'reference')['status'] == 'learning_positive'
    assert len(performance_decision(rows, 'reference')['cells']) == 45
    bad = copy.deepcopy(rows)
    for seed in SEEDS['confirmation']:
        for arm in ('jitter_256', 'jitter_1024'):
            bad[f'{arm}/{seed}']['fixtures']['core']['coordinates']['switch_noisy']['post_mse'] = 99.
    assert tolerance_point(bad)['tolerance'] == 32
    for control in ('window', 'sgd', 'adwin'):
        for label, fields in SPECS:
            ...
            result = performance_decision(bad, 'reference')
    ...
    for key in ('adwin/173000', 'reference/173000', 'jitter_256/173000', 'oracle_nofallback/173000'):
        rejects(lambda k=key: performance_decision({a: r for a, r in rows.items() if a != k}, 'reference'))
    bad = copy.deepcopy(rows)
    for seed in SEEDS['confirmation']: bad[f'reference/{seed}']['status'] = 'nonfinite'
    assert performance_decision(bad, 'reference')['status'] == 'learning_negative'
    print('PASS drifttol policy: frozen menus, 45-cell instrument, tolerance point', flush=True)
...
def lifecycle_checks():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('study_v3_drifttol.require_committed', side_effect=ValueError('uncommitted')): rejects(lambda: confirm(directory))
        with patch('study_v3_drifttol.require_committed'):
        ...
        broken = copy.deepcopy(rows); broken['jitter_256/173000']['manifest_digest'] = 'wrong'
        ...
        broken = copy.deepcopy(rows); broken['oracle_nofallback/173000']['privileged_timing'] = False
        envelope('confirmation', broken); rejects(lambda: read_stage(directory, 'confirmation'))
        broken = copy.deepcopy(rows); broken['reference/173000']['memory']['core']['quiet']['resets'][0]['kind'] = 'random'
        rejects(lambda: absolute_summary(broken))
        broken = copy.deepcopy(rows); broken['reference/173000']['memory']['core']['quiet']['regime_slope_share'] = 2.
        rejects(lambda: absolute_summary(broken))
        envelope('confirmation', rows)
        for k in tuning:
        ...
        with patch('study_v3_drifttol.require_committed'): rejects(lambda: confirm(directory))
    ...
    for arm in ARMS:
        config = GRIDS[arm][0] if arm in FAMILIES else dict(ESTIMATOR) if arm != 'oracle_nofallback' else dict(FAST)
        row = measure_row(170002, arm, config, None)
    ...
    print('PASS drifttol lifecycle: provenance accounting, source/manifest/stage gates, complete reports, all-arm development fixtures', flush=True)
...
def archive_checks(directory, reproduce):
    ...
    print('PASS drifttol archive tuning', len(tuning), flush=True)
    if not reproduce:
    ...
    for arm in FAMILIES:
        ...
        print('PASS drifttol reproduction tuning', arm, '96 rows', flush=True)
    ...
    if stored['counts']['confirmation']:
        ...
        for arm in ARMS:
            for seed in SEEDS['confirmation']:
                key = f'{arm}/{seed}'
                recreated[key] = measure_row(seed, arm, fresh_manifest['screen']['selected'][arm], None, fresh_manifest)
                compare(recreated[key], scientific(archived[key]), key, ignore=('manifest_digest',))
            print('PASS drifttol reproduction confirmation', arm, '32 rows', flush=True)
        for arm in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50', 'oracle_nofallback'):
    ...
    print('PASS full drifttol reproduction; only reached partitions sampled', flush=True)
