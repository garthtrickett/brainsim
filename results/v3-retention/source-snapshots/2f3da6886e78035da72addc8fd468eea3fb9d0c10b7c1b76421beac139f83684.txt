"""Registered separate-reference gates; fixed policies and complete seed partitions."""
import hashlib
import json
import math
...
from v3_reference import CONFIGS, DETECTORS

PROTOCOL = {'id': 'v3-reference-20260909-v1',
            'registration_commit': '8c72ef5e46e14fd22d04d84486176535f82b77c0',
            'registration_sha256': '0d85fa18f4c636e4e93cb7f6d9cc5ef763fac8a7c2e0ccc0ee03ede28cef8341'}
SEEDS = {'development': tuple(range(60000, 60008)), 'calibration': tuple(range(61000, 61016)),
         'diagnostics': tuple(range(63000, 63032)), 'performance': tuple(range(64000, 64032))}
CONTROLS = tuple(a for a in CONFIGS if a != 'separate')
...
def interval(values):
    ...
    indices = np.random.default_rng(65000).integers(0, len(values), (10000, len(values)))
...
def detector_decision(rows, arm='separate', common=False):
    ...
    field = 'common_modes' if common else 'modes'
    cells = {}
    ...
    for mode in ('fixed', 'closed'):
        for coord in ('quiet', 'noisy'):
            metrics = [rows[f'{arm}/{s}'][field][mode]['core'][coord] for s in SEEDS['diagnostics']]
        ...
        for name, coord, kinds, detection in specs:
            for kind in kinds:
                ...
                for seed in SEEDS['diagnostics']:
                    events = rows[f'{arm}/{seed}'][field][mode][name][coord]['events']
...
def performance_decision(rows):
    ...
    cells = {f'{arm}/{label}': contrast(values(arm, fields), values('separate', fields), improvement)
