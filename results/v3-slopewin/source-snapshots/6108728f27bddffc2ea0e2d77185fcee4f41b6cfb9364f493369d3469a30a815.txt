...
from v3_drift import ARMS, FAMILIES, GRIDS, random_uniforms, run
from v3_drift_policy import KEYS, PROTOCOL, SEEDS, complete, digest, finite, select
from v3_persistent import FIXTURES, fixture
from v3_slice1_streams import learning_metrics

DIRECTORY = Path('results/v3-drift')
SOURCES = sorted(set(BURST_SOURCES + ['DESIGN-v3-drift.md', 'v3_drift.py',
    'v3_adwin.py', 'v3_forgetting.py', 'v3_retention.py', 'v3_retention_policy.py',
    'report_v3_retention.py', 'study_v3_retention.py', 'v3_drift_policy.py', 'study_v3_drift.py',
...
def registration_check():
    if source_hashes()['DESIGN-v3-drift.md'] != PROTOCOL['registration_sha256']:
...
def enriched_schedule(data):
    """True abrupt-target times plus drift boundaries, evaluator-owned only."""
...
def boundary_kind(data, index, coordinate):
    for start, kind in data.events[coordinate]:
        if start == index:
            if kind.startswith('target_'):
                return 'target'
            if kind in ('drift_start', 'drift_end'):
                return 'drift'
    return 'unknown'
...
def memory_metrics(data, result, arm):
    ...
    for j, coordinate in enumerate(data.coordinates):
        ...
        if arm in FAMILIES:
            ...
            record = window_record(flags, result[3][:, j], result[4][:, j], data.events[j])
            record['kind'] = 'dual' if arm in ('retain_drift', 'random_drift') else 'window'
            ...
            kinds = [boundary_kind(data, t, j) if arm != 'random_drift' else 'random'
...
def simulation(seed, name, arm, config, probability=None):
    ...
    kwargs = {}
    if arm in ('retain_drift', 'oracle_nofallback'):
    ...
    for arm in ARMS:
        ...
        for seed in SEEDS['confirmation']:
            evidence.measure(f'{arm}/{seed}', lambda s=seed, a=arm, c=config:
                             measure_row(s, a, c, probability if a == 'random_drift' else None, manifest))
...
def main():
    ...
    if args.stage == 'report':
        from report_v3_drift import publish_report
