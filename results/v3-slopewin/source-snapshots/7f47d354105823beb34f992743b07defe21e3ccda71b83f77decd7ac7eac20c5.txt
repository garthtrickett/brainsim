"""Frozen slope menus and the 75-cell gate with the repaired both-perfect veto."""
import numpy as np

from v3_burst import random_probability
from v3_retention_policy import SPECS, STRICT, contrast, digest, finite, metric_values, objective, scientific
from v3_slope import ARMS, FAMILIES, FAST, GRIDS

PROTOCOL = {'id': 'v3-slope-20260909-v1',
            'registration_commit': 'a2bdde25fc5dce651ab6ae710334da8d00fde928',
            'registration_sha256': 'c089f89083e200ef4142d4330c8fc01309767820f9331feea2c37433d088fe3f'}
SEEDS = {'development': tuple(range(130000, 130008)), 'tuning': tuple(range(131000, 131008)),
         'confirmation': tuple(range(133000, 133032))}
...
def select(rows):
    ...
    if not reasons:
        chosen = [rows[f'slope/{indices["slope"]}/{s}'] for s in SEEDS['tuning']]
        ...
        selected['random_slope'] = dict(selected['slope'])
    return result
...
def interval(values):
    ...
    indices = np.random.default_rng(135000).integers(0, len(values), (10000, len(values)))
