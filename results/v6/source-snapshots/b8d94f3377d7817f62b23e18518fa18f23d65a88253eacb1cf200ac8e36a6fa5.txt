...
class BrainSim:
    ...
    WM_HOLD     = False
    # Step 9: local competitive pools. One global k-WTA makes every stimulus
    # compete with every other, which is the interference the step 7 reference
    # table exposed -- xor-2 and volatile-4 both lose to tabular-Q, the ONE
    # baseline with a separate entry per state and so zero interference.
    # POOLS>1 splits the hidden layer into P blocks each running its own k-WTA
    # at k/P. P must divide k; balanced pool sizes may differ by one unit.
    # The original P=4/8 probe silently changed k=6 into 4/8 winners.
    POOLS       = 1
    # Step 13c instruments. Transmission failure and matched deterministic
    # attenuation default off. Masks use their own RNG, preserving the baseline.
    TRANSMISSION_FAILURE = 0.0
    ...
    def __init__(self, n_in=40, n_hidden=80, n_motor=2, k=6, seed=0):
        ...
        self.transmission_rng = np.random.default_rng([seed, 1303])
    ...
    def _validate_pools(self):
        if (not isinstance(self.POOLS, (int, np.integer)) or self.POOLS < 1
    ...
    def _sample_episode(self):
        """Selection seam: experiments can change retrieval without copying TD replay."""
