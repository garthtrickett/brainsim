...
from v3_slopeguard import ARMS, FAMILIES, GRIDS, run
from v3_slopeguard_policy import KEYS, PROTOCOL, SEEDS, complete, digest, finite, select
from v3_slice1_streams import learning_metrics

DIRECTORY = Path('results/v3-slopeguard')
SOURCES = sorted(set(BURST_SOURCES + ['DESIGN-v3-slopeguard.md', 'v3_slopeguard.py',
    ...
    'report_v3_drift.py', 'study_v3_drift.py', 'v3_slopebench.py', 'v3_slopebench_policy.py',
    'report_v3_slopebench.py', 'study_v3_slopebench.py', 'v3_slopeguard_policy.py', 'study_v3_slopeguard.py',
    'report_v3_slopeguard.py', 'check_v3_slopeguard.py']))
...
def registration_check():
    if source_hashes()['DESIGN-v3-slopeguard.md'] != PROTOCOL['registration_sha256']:
...
def memory_metrics(data, result, arm):
    ...
    for j, coordinate in enumerate(data.coordinates):
        ...
        if arm in ('window', 'adwin'):
            ...
            record = window_record(flags, result[3][:, j], result[4][:, j], data.events[j])
            record['kind'] = 'dual' if arm in ('guard', 'random_guard') else 'window'
...
def simulation(seed, name, arm, config, probability=None):
    ...
    kwargs = {}
    if arm in ('guard', 'reference', 'oracle_nofallback'):
        kwargs['schedule'] = enriched_schedule(data)
        if arm in ('guard', 'reference'):
    ...
    for arm in ARMS:
        ...
        for seed in SEEDS['confirmation']:
            evidence.measure(f'{arm}/{seed}', lambda s=seed, a=arm, c=config:
                             measure_row(s, a, c, probability if a == 'random_guard' else None, manifest))
...
def main():
    ...
    if args.stage == 'report':
        from report_v3_slopeguard import publish_report
