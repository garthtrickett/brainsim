"""Complete drift-boundary results with target/drift provenance accounting."""
import argparse
import json
...
from study_v3_drift import DIRECTORY, make_manifest, read_stage
from v3_drift import ARMS, GRIDS
from v3_drift_policy import (KEYS, SEEDS, digest, interval, metric_values,
            ...
            for coord in values[0]['coordinates']:
                ...
                for record in memory:
                    ...
                    if arm == 'sgd':
                        ...
                        if record['kind'] != ('dual' if arm in ('retain_drift', 'random_drift') else 'window'):
                            raise ValueError('invalid memory kind')
                        check_widths(record)
                        for reset in record['resets']:
                            ...
                            if arm in ('retain_drift', 'oracle_nofallback', 'random_drift'):
                                if reset.get('kind') not in ('target', 'drift', 'random'):
                                    raise ValueError('mixed boundary provenance')
                                if arm == 'random_drift' and reset['kind'] != 'random':
                                    raise ValueError('wrong boundary provenance')
                                if arm in ('retain_drift', 'oracle_nofallback') and reset['kind'] not in ('target', 'drift'):
                                    raise ValueError('wrong boundary provenance')
                        if record['kind'] == 'dual':
                ...
                dual = arm in ('retain_drift', 'random_drift')
...
def summarize(directory):
    ...
    if (directory/'confirmation.json').exists():
        ...
        result['decisions'] = {'retain_drift': performance_decision(rows)}
        result['status'], result['closed'] = result['decisions']['retain_drift']['status'], True
...
def markdown(summary):
    lines = ['# V3 drift-boundary schedule', '', f'Disposition: **{summary["status"]}**.', '',
             '| Stage | Rows |', '| --- | ---: |']
    ...
    lines += ['', '## Finite-menu tuning', '', 'Each searched family: 12 configurations ×8 seeds ×5 fixtures. '
              ...
              'The candidate configuration is frozen from the retention manifest; enriched-schedule arms '
              'run in confirmation only.', '',
    ...
    lines += ['', 'Endpoint choices remain in the registered finite menu; no extension or global-optimum claim. '
              'Oracle_nofallback is the fixed fast base on the enriched schedule. Random_drift uses the frozen '
    ...
    for arm, decision in summary['decisions'].items():
        lines += ['', f'## {arm}: {decision["status"]}', '',
                  'All75 comparisons are required: >=10% primary improvement over window/sgd/adwin/random_drift, '
                  'primary preservation against oracle_nofallback, every retention bound, and strict improvement on '
                  'the8 stable/noise cells against oracle_nofallback (both-perfect cells pass as preservation).', '',
    ...
    if summary['absolute']:
        ...
        lines += ['', 'Width counts retained observations, not physical storage. SGD uses exponential weights '
                  'and has no literal window. Target and drift-boundary requests are distinguished by provenance '
                  'in every record and never mix; random requests carry random provenance. Fast share is the '
    ...
    lines += ['', 'Configuration budgets are equal, not CPU costs. Times exclude archive writes and share '
              ...
              'Granted timing is privileged, not perfect segmentation: retaining K_fast=4 at any boundary may '
              'retain old-regime data. No schedule outcome overrides candidate performance. '
              'Candidate success requires all75 comparisons. All outcomes close this registration.', '',
              'Intervals resample whole seeds10000 times with seed125000. Finite menus and synthetic fixtures '
              ...
              '`python check_v3_drift.py --evidence results/v3-drift --reproduce` regenerates every '
              'reached row and scientific manifest/summary at rtol1e-11/atol1e-13. '
              '`python report_v3_drift.py --check` verifies both generated reports.', '']
    return '\n'.join(lines)
