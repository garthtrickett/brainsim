"""Complete slope results with trend-regime and boundary accounting."""
import argparse
import json
...
from study_v3_slope import DIRECTORY, make_manifest, read_stage
from v3_slope import ARMS, GRIDS
from v3_slope_policy import (KEYS, SEEDS, digest, interval, metric_values,
            ...
            for coord in values[0]['coordinates']:
                ...
                for record in memory:
                    ...
                    if arm == 'sgd':
                        ...
                        if record['kind'] != ('dual' if arm in ('slope', 'random_slope') else 'window'):
                            raise ValueError('invalid memory kind')
                        check_widths(record)
                        for reset in record['resets']:
                            ...
                            if arm in ('slope', 'oracle_nofallback', 'random_slope'):
                                if reset.get('kind') not in ('target', 'drift', 'random'):
                                    raise ValueError('mixed boundary provenance')
                                if arm == 'random_slope' and reset['kind'] != 'random':
                                    raise ValueError('wrong boundary provenance')
                                if arm in ('slope', 'oracle_nofallback') and reset['kind'] not in ('target', 'drift'):
                                    raise ValueError('wrong boundary provenance')
                        if record['kind'] == 'dual':
                ...
                dual = arm in ('slope', 'random_slope')
...
def summarize(directory):
    ...
    if (directory/'confirmation.json').exists():
        ...
        result['decisions'] = {'slope': performance_decision(rows)}
        result['status'], result['closed'] = result['decisions']['slope']['status'], True
...
def markdown(summary):
    lines = ['# V3 slope state', '', f'Disposition: **{summary["status"]}**.', '',
             '| Stage | Rows |', '| --- | ---: |']
    ...
    lines += ['', '## Finite-menu tuning', '', 'Each searched family: 12 configurations ×8 seeds ×5 fixtures. '
              ...
              'The dual base is frozen; the estimator family alone is searched.', '',
    ...
    lines += ['', 'Endpoint choices remain in the registered finite menu; no extension or global-optimum claim. '
              'Oracle_nofallback is the fixed fast base on the enriched schedule. Random_slope uses the selected estimator.']
    ...
    for arm, decision in summary['decisions'].items():
        lines += ['', f'## {arm}: {decision["status"]}', '',
                  'All75 comparisons are required: >=10% primary improvement over window/sgd/adwin/random_slope, '
    ...
    if summary['absolute']:
        ...
        lines += ['', 'Width counts retained observations, not physical storage. SGD uses exponential weights '
                  ...
                  'provenance in every record and never mix. Regime shares are post-burn fractions; slope mean '
                  'averages the estimator slope over in-regime post-burn steps (0 when the regime never fires).', '']
    ...
    lines += ['', 'Configuration budgets are equal, not CPU costs. Times exclude archive writes and share '
              ...
              'Intervals resample whole seeds10000 times with seed135000. Finite menus and synthetic fixtures '
              ...
              '`python check_v3_slope.py --evidence results/v3-slope --reproduce` regenerates every '
              'reached row and scientific manifest/summary at rtol1e-11/atol1e-13. '
              '`python report_v3_slope.py --check` verifies both generated reports.', '']
    return '\n'.join(lines)
