...
from v3_slopetime import ARMS, FAMILIES, GRIDS, random_uniforms, run
from v3_slopetime_policy import KEYS, PROTOCOL, SEEDS, TUNING_ALARM, complete, digest, finite, select
from v3_slice1_streams import learning_metrics

DIRECTORY = Path('results/v3-slopetime')
SOURCES = sorted(set(BURST_SOURCES + ['DESIGN-v3-slopetime.md', 'v3_slopetime.py',
    ...
    'v3_drift.py', 'v3_drift_policy.py', 'report_v3_drift.py', 'study_v3_drift.py',
    'v3_slopetime_policy.py', 'study_v3_slopetime.py',
    'report_v3_slopetime.py', 'check_v3_slopetime.py']))
...
def registration_check():
    if source_hashes()['DESIGN-v3-slopetime.md'] != PROTOCOL['registration_sha256']:
...
def alarm_schedule(data, delta, clock):
    """Observed alarms from the standalone control run; observables only."""
    return adwin_run(data.y, delta, clock)[1]
...
def memory_metrics(data, result, arm, provenance):
    ...
    for j, coordinate in enumerate(data.coordinates):
        ...
        if arm in ('window', 'adwin'):
            ...
            record = window_record(flags, result[3][:, j], result[4][:, j], data.events[j])
            record['kind'] = 'dual' if arm in ('slope_adwin', 'random_slope') else 'window'
            ...
            if arm == 'reference':
                kinds = [boundary_kind(data, t, j) for t in np.flatnonzero(flags).tolist()]
            elif arm == 'random_slope':
                kinds = ['random']*int(flags.sum())
            else:
                kinds = ['observed']*int(flags.sum())
...
def simulation(seed, name, arm, config, alarm=None, probability=None):
    data = observations(seed, name)
    kwargs = {}
    if arm in ('slope_adwin', 'oracle_nofallback'):
        if alarm is None:
            raise ValueError('generating control configuration required')
        kwargs['schedule'] = alarm_schedule(data, alarm['delta'], alarm['clock'])
    elif arm == 'reference':
...
def measure_row(seed, arm, config, alarm=None, probability=None, manifest=None):
    row = {'status': 'ok', 'config': config, 'privileged_timing': arm not in ('window', 'sgd', 'adwin'),
           'fixtures': {}, 'memory': {}}
    if manifest is not None:
        row['manifest_digest'] = digest(manifest)
    for name in FIXTURES:
        data, result = simulation(seed, name, arm, config, alarm, probability)
        ...
        row['memory'][name] = memory_metrics(data, result, arm,
            'none' if arm in ('window', 'sgd', 'adwin') else 'observed' if arm in ('slope_adwin', 'oracle_nofallback')
            else 'true' if arm == 'reference' else 'random')
    if not finite(row):
    ...
    for arm in FAMILIES:
        for i, config in enumerate(GRIDS[arm]):
            for seed in SEEDS['tuning']:
                if arm == 'slope_adwin':
                    evidence.measure(f'{arm}/{i}/{seed}',
                        lambda s=seed, a=arm, c=config: measure_row(s, a, c, TUNING_ALARM))
                else:
    ...
    for arm in ARMS:
        ...
        for seed in SEEDS['confirmation']:
            evidence.measure(f'{arm}/{seed}', lambda s=seed, a=arm, c=config:
                             measure_row(s, a, c, alarm if a in ('slope_adwin', 'oracle_nofallback') else None,
                                         probability if a == 'random_slope' else None, manifest))
...
def main():
    ...
    if args.stage == 'report':
        from report_v3_slopetime import publish_report
