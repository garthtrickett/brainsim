...
from v3_reference import CONFIGS, DETECTORS, FIXTURES, fixture, run, warmup
from v3_reference_policy import (KEYS, PROTOCOL, SEEDS, complete, detector_decision,
                                   digest, finite)
from v3_slice1_streams import BURN, diagnostic_metrics, learning_metrics, maxima, recovery

DIRECTORY = Path('results/v3-reference')
SOURCES = ['DESIGN-v3-reference.md', 'DESIGN-v3-persistent.md', 'DESIGN-v3-slice1.md',
           'requirements.txt', 'v3_reference.py', 'v3_reference_policy.py', 'study_v3_reference.py',
           'report_v3_reference.py', 'v3_persistent.py', 'study_io.py', 'study_v3_slice1.py',
...
def registration_check():
    if source_hashes()['DESIGN-v3-reference.md'] != PROTOCOL['registration_sha256']:
...
def explanatory_events(data, predictions, gates):
    records = {}
    for j, coordinate in enumerate(data.coordinates):
        error = predictions[:, j]-data.target[:, j]
        records[coordinate] = []
        for start, kind in data.events[j]:
            if not kind.startswith('target_'):
                continue
            latency, success = recovery(error, start)
            records[coordinate].append({
                'index': start, 'kind': kind, 'q_max100': float(gates[start:start+100, j].max()),
                'error_at_event': float(error[start]), 'mse_before100': float(np.mean(error[start-100:start]**2)),
                'mse_after20': float(np.mean(error[start:start+20]**2)),
                'mse_after100': float(np.mean(error[start:start+100]**2)),
                'mse_after200': float(np.mean(error[start:start+200]**2)),
                'recovery_latency': latency, 'recovery_success': success,
                # All ten good predictions must fit within the first 100 observations.
                'recovered_within_100': bool(success and latency+10 <= 100)})
    return records


def diagnostic_row(seed, arm, manifest):
    modes, common, learning, events = {}, {}, {}, {}
    identity = digest(manifest)
    for mode in ('fixed', 'closed'):
        modes[mode], common[mode], learning[mode], events[mode] = {}, {}, {}, {}
        for name in FIXTURES:
            ...
            common[mode][name] = diagnostic_metrics(data, result[1], manifest['thresholds']['separate']['fixed'])
            with np.errstate(over='ignore', invalid='ignore'):
                learning[mode][name] = learning_metrics(data, *result)
                events[mode][name] = explanatory_events(data, result[0], result[1])
            if not finite(learning[mode][name]) or not finite(events[mode][name]):
                return {'status': 'nonfinite', 'manifest_digest': identity, 'reason': mode+'/'+name+' metrics'}
    return {'status': 'ok', 'manifest_digest': identity, 'modes': modes, 'common_modes': common,
            'learning': learning, 'explanatory_events': events}
...
def make_manifest(rows):
    ...
    if manifest['status'] == 'eligible':
        ...
        manifest['constants'] = np.mean([rows[f'separate/{s}']['modes']['closed']['gate_means']
...
def main():
    ...
    if args.stage == 'report':
        from report_v3_reference import publish_report
