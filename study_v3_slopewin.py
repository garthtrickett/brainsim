...
from v3_slopewin import ARMS, FAMILIES, GRIDS, random_uniforms, run
from v3_slopewin_policy import KEYS, PROTOCOL, SEEDS, complete, digest, finite, select
from v3_slice1_streams import learning_metrics

DIRECTORY = Path('results/v3-slopewin')
SOURCES = sorted(set(BURST_SOURCES + ['DESIGN-v3-slopewin.md', 'v3_slopewin.py',
    ...
    'report_v3_slopebench.py', 'study_v3_slopebench.py', 'v3_slopewin_policy.py', 'study_v3_slopewin.py',
...
def registration_check():
    if source_hashes()['DESIGN-v3-slopewin.md'] != PROTOCOL['registration_sha256']:
...
def memory_metrics(data, result, arm):
    ...
    for j, coordinate in enumerate(data.coordinates):
        ...
        if arm in ('window', 'adwin'):
            ...
            record = window_record(flags, result[3][:, j], result[4][:, j], data.events[j])
            record['kind'] = 'dual' if arm in ('win', 'random_win') else 'window'
            if arm in ('win', 'random_win'):
                ...
                record.update(adwin_width_mean=float(np.mean(slow_widths[1000:])),
                    ...
                    win_mean=float(result[9][1000:, j][result[7][1000:, j] == 2].mean())
                    if (result[7][1000:, j] == 2).any() else 0.)
            kinds = [boundary_kind(data, t, j) if arm != 'random_win' else 'random'
...
def simulation(seed, name, arm, config, probability=None):
    ...
    kwargs = {}
    if arm in ('win', 'reference', 'oracle_nofallback'):
        kwargs['schedule'] = enriched_schedule(data)
        if arm in ('win', 'reference'):
    ...
    for arm in ARMS:
        ...
        for seed in SEEDS['confirmation']:
            evidence.measure(f'{arm}/{seed}', lambda s=seed, a=arm, c=config:
                             measure_row(s, a, c, probability if a == 'random_win' else None, manifest))
...
def main():
    ...
    if args.stage == 'report':
        from report_v3_slopewin import publish_report
