"""Tolerance curve plus the deployable ADWIN-schedule gate, kept separate."""
import argparse
import json
...
from study_v3_tolerance import DIRECTORY, make_manifest, read_stage
from v3_tolerance import ARMS, GRIDS
from v3_tolerance_policy import (KEYS, LEVELS, SEEDS, digest, interval, metric_values,
                                 performance_decision, tolerance_point)
from v3_persistent import FIXTURES

MEASURED = ['reference'] + LEVELS + ['drop_10', 'drop_50', 'add_0005', 'add_0020']
        ...
        for name in FIXTURES:
            ...
            for coord in values[0]['coordinates']:
                ...
                for record in memory:
                    ...
                    if record['provenance'] not in ('none', 'true', 'observed', 'jitter_2', 'jitter_8',
                            'jitter_16', 'jitter_32', 'jitter_128', 'drop_10', 'drop_50', 'add_0005', 'add_0020'):
                        raise ValueError('mixed schedule provenance')
                    expected = ('none' if arm in ('window', 'sgd', 'adwin') else 'observed'
                                if arm == 'adwin_schedule' else 'true' if arm == 'reference' else arm)
                    ...
                    if arm == 'sgd':
                        ...
                        if record['kind'] != ('dual' if arm not in ('window', 'adwin') else 'window'):
                            raise ValueError('invalid memory kind')
                        check_widths(record)
                ...
                dual = arm not in ('window', 'sgd', 'adwin')
...
def summarize(directory):
    ...
    if (directory/'confirmation.json').exists():
        ...
        result['decisions'] = {a: performance_decision(rows, a) for a in MEASURED + ['adwin_schedule']}
        result['tolerance'] = tolerance_point(rows)
        result['status'], result['closed'] = result['decisions']['adwin_schedule']['status'], True
...
def contrast_table(lines, decision):
...
def markdown(summary):
    lines = ['# V3 timing tolerance', '', f'Disposition: **{summary["status"]}** (ADWIN-schedule arm).', '',
             '| Stage | Rows |', '| --- | ---: |']
    ...
    lines += ['', '## Finite-menu tuning', '', 'Each searched family: 12 configurations ×8 seeds ×5 fixtures. '
              ...
              'The candidate configuration is frozen from the retention manifest; perturbed and '
              'ADWIN-schedule arms are schedule variants run in confirmation only.', '',
    ...
    lines += ['', 'Endpoint choices remain in the registered finite menu; no extension or global-optimum claim.',
              '', *screen['reasons']]
    if summary['tolerance'] is not None:
        lines += ['', '## Tolerance measurement (diagnostic families)', '',
                  'Per-level 45-comparison instrument against window/SGD/ADWIN. '
                  'The tolerance point is the largest jitter passing all45. This disposition authorizes nothing.', '',
                  '| Arm | Primary MSE | Cells passed /45 | Status |', '| --- | ---: | ---: | --- |']
        for arm in MEASURED:
            m = summary['absolute'][arm].get('metrics', {}).get('primary')
            passed = sum(c['pass'] for c in summary['decisions'][arm]['cells'].values())
            lines.append(f'| {arm} | {number(m["mean"] if m else None)} | {passed}/45 | {summary["decisions"][arm]["status"]} |')
        lines += ['', f'Tolerance point: **{summary["tolerance"]["tolerance"]}**.']
    lines += ['', '## adwin_schedule: ' + summary['decisions'].get('adwin_schedule', {}).get('status', 'pending'), '',
              'The deployable arm faces the same45 comparisons. All45 must pass for learning_positive; '
              'a positive justifies only a separately authorized broader benchmark, never agent integration.', '']
    if 'adwin_schedule' in summary['decisions']:
        contrast_table(lines, summary['decisions']['adwin_schedule'])
    if 'reference' in summary['decisions']:
        lines += ['', '## reference (unperturbed schedule, diagnostic)', '']
        contrast_table(lines, summary['decisions']['reference'])
    if summary['absolute']:
        ...
        lines += ['', 'Width counts retained observations, not physical storage. SGD uses exponential weights '
                  'and has no literal window. True, jittered, dropped, added and observed schedules are '
                  'distinguished by provenance in every record and never mix. Fast share is the post-burn '
                  'fraction of predictions taken from the fast window.', '']
    ...
    lines += ['', 'Configuration budgets are equal, not CPU costs. Times exclude archive writes and share '
              ...
              'Perturbed schedules are diagnostic: jittered/dropped/added times are evaluator-built from true '
              'times, not observable-derived. Only the ADWIN-schedule arm is deployable. No schedule outcome '
              'overrides the advancement gate. The tolerance curve authorizes no follow-up; the reading rules '
              'in the registration decide what follows.', '',
              'Intervals resample whole seeds10000 times with seed115000. Finite menus and synthetic fixtures '
              ...
              '`python check_v3_tolerance.py --evidence results/v3-tolerance --reproduce` regenerates every '
              'reached row and scientific manifest/summary at rtol1e-11/atol1e-13. '
              '`python report_v3_tolerance.py --check` verifies both generated reports.', '']
    return '\n'.join(lines)
...
def publish_report(directory, check=False):
    ...
    print('RESULT', summary['status'], summary['counts'], summary['tolerance'], flush=True)
    return summary
