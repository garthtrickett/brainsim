"""Kink-guard results with four-regime accounting."""
import argparse
import json
...
from study_v3_slopeguard import DIRECTORY, make_manifest, read_stage
from v3_slopeguard import ARMS, GRIDS
from v3_slopeguard_policy import (KEYS, SEEDS, digest, interval, metric_values,
            ...
            for coord in values[0]['coordinates']:
                ...
                for record in memory:
                    ...
                    if arm == 'sgd':
                        ...
                        if record['kind'] != ('dual' if arm in ('guard', 'random_guard') else 'window'):
                            raise ValueError('invalid memory kind')
                        check_widths(record)
                        for reset in record['resets']:
                            ...
                            if arm in ('guard', 'reference', 'oracle_nofallback', 'random_guard'):
                                if reset.get('kind') not in ('target', 'drift', 'random'):
                                    raise ValueError('mixed boundary provenance')
                                if arm == 'random_guard' and reset['kind'] != 'random':
                                    raise ValueError('wrong boundary provenance')
                                if arm in ('guard', 'reference', 'oracle_nofallback') and reset['kind'] not in ('target', 'drift'):
                                    raise ValueError('wrong boundary provenance')
                        if record['kind'] == 'dual':
                            check_widths(record, 'adwin_')
                            for field in ('regime_fast_share', 'regime_guard_share', 'regime_slope_share'):
                                share = record[field]
                                if type(share) is not float or not 0. <= share <= 1.:
                                    raise ValueError('invalid regime share')
                            if record['regime_fast_share']+record['regime_guard_share']+record['regime_slope_share'] > 1.+1e-9:
                ...
                dual = arm in ('guard', 'random_guard')
                for field in ('adwin_width_mean', 'adwin_width_final', 'adwin_total_discarded',
                              'regime_fast_share', 'regime_guard_share', 'regime_slope_share', 'slope_mean'):
...
def summarize(directory):
    ...
    if (directory/'confirmation.json').exists():
        ...
        result['decisions'] = {'guard': performance_decision(rows)}
        result['status'], result['closed'] = result['decisions']['guard']['status'], True
...
def markdown(summary):
    lines = ['# V3 kink-guarded slope', '', f'Disposition: **{summary["status"]}**.', '',
             '| Stage | Rows |', '| --- | ---: |']
    ...
    lines += ['', '## Finite-menu tuning', '', 'Each searched family: 12 configurations ×8 seeds ×5 fixtures. '
              ...
              'Only the guard horizon is searched; estimator and base are frozen.', '',
    ...
    lines += ['', 'Endpoint choices remain in the registered finite menu; no extension or global-optimum claim. '
              'Reference is the no-guard slope policy. Random_guard uses the selected guard horizon.']
    ...
    for arm, decision in summary['decisions'].items():
        lines += ['', f'## {arm}: {decision["status"]}', '',
                  'All75 comparisons are required: >=10% primary improvement over window/sgd/adwin/random_guard, '
    ...
    if summary['absolute']:
            ...
            for name, data in row.get('fixtures', {}).items():
                for coord, c in data['coordinates'].items():
                    fields = [c['excess_mse'], c['post_mse'], c['stable_mse'], c['ramp_mse'], c['adaptation_latency'],
                              ...
                              c['adwin_width_mean'], c['regime_fast_share'], c['regime_guard_share'], c['regime_slope_share'], c['slope_mean']]
        ...
        lines += ['', 'Width counts retained observations, not physical storage. SGD uses exponential weights '
                  'and has no literal window. Target, drift-boundary and random requests are distinguished by '
                  'provenance in every record and never mix. Guard share counts in-segment fast predictions; '
                  'fast share counts out-of-segment ones.', '']
    ...
    lines += ['', 'Configuration budgets are equal, not CPU costs. Times exclude archive writes and share '
              ...
              'Intervals resample whole seeds10000 times with seed165000. Finite menus and synthetic fixtures '
              ...
              '`python check_v3_slopeguard.py --evidence results/v3-slopeguard --reproduce` regenerates every '
              'reached row and scientific manifest/summary at rtol1e-11/atol1e-13. '
              '`python report_v3_slopeguard.py --check` verifies both generated reports.', '']
    return '\n'.join(lines)
