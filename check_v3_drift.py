"""Enriched-schedule equations, repaired veto authority and full reproduction."""
import argparse
import ast
import copy
import hashlib
...
from report_v3_drift import absolute_summary, publish_report, summarize
from study_v3_drift import (SOURCES, boundary_kind, confirm, enriched_frequency, enriched_schedule,
    make_manifest, measure_row, read_stage, registration_check, require_committed, simulation,
...
from v3_drift import ARMS, FAMILIES, FAST, FROZEN, GRIDS, random_uniforms, run
from v3_retention import dual_run
from v3_drift_policy import (KEYS, PROTOCOL, SEEDS, SPECS, complete, contrast,
...
def kernel_checks():
    registration_check()
    assert FROZEN == {'delta': .1, 'H': 32} and FAST == {'keep': 4, 'window': 32}
    ...
    assert all(min(s) >= 120000 and max(s) < 126000 for s in parts)
    ...
    values = np.random.default_rng(120001).normal(size=600)
    ...
    # Enriched schedule: targets plus drift boundaries, nothing else, mapped kinds.
    y = np.random.default_rng(120000).normal(size=(500, 2))
    for name in FIXTURES:
        data = fixture(120001, name)
        schedule = enriched_schedule(data)
        assert schedule.shape == data.y.shape and set(np.unique(schedule)) <= {0., 1.}
        marked = {(t, j) for j, ev in enumerate(data.events) for t, k in ev
                  if k.startswith('target_') or k in ('drift_start', 'drift_end')}
        assert {tuple(v) for v in np.argwhere(schedule)} == marked
        noisy = {(t, j) for j, ev in enumerate(data.events) for t, k in ev if k.startswith('noise_')}
        assert not ({tuple(v) for v in np.argwhere(schedule)} & noisy)
        for j in range(len(data.coordinates)):
            for t in np.flatnonzero(schedule[:, j]).tolist():
                assert boundary_kind(data, t, j) in ('target', 'drift')
        assert np.array_equal(schedule, enriched_schedule(data))
        prefix = fixture(120001, name)
        assert np.array_equal(enriched_schedule(prefix)[:300], schedule[:300])
    assert boundary_kind(fixture(120001, 'core'), 0, 0) == 'unknown'
    np.testing.assert_allclose(random_probability(80, 432000, 1), 0.00018573551263001485, rtol=1e-14)
    # Frozen parity: enriched arms are the retention dual-state at FROZEN.
    triggers = np.zeros_like(y); triggers[200] = 1.
    for a, b in zip(run(y, 'retain_drift', dict(FROZEN), schedule=triggers),
    ...
    uniforms = random_uniforms(120000, 0, len(y), 2)
    for arm in ARMS:
        config = GRIDS[arm][3] if arm in FAMILIES else dict(FAST) if arm == 'oracle_nofallback' else dict(FROZEN)
        if arm in FAMILIES:
            kwargs = {}
        elif arm == 'random_drift':
        ...
        if arm in FAMILIES:
            ...
            rejects(lambda a=arm, c=config: run(y, a, c, uniforms=uniforms, probability=.01))
    np.testing.assert_array_equal(uniforms[:200], random_uniforms(120000, 0, 200, 2))
    np.testing.assert_array_equal(uniforms[:, :1], random_uniforms(120000, 0, len(y), 1))
    assert not np.array_equal(uniforms, random_uniforms(120000, 1, len(y), 2))
    assert not np.array_equal(uniforms[:, 0], uniforms[:, 1])
    rejects(lambda: run(y, 'retain_drift', {'delta': .1, 'H': 0}, schedule=triggers))
    rejects(lambda: run(y, 'oracle_nofallback', {'keep': 4, 'window': 64}, schedule=triggers))
    rejects(lambda: run(np.full_like(y, np.nan), 'window', {'window': 16}))
    rejects(lambda: run(y, 'retain_drift', dict(FROZEN)))
    rejects(lambda: run(y, 'random_drift', dict(FROZEN), uniforms=uniforms, probability=2.))
    for name in FIXTURES:
        with patch('study_v3_drift.enriched_schedule', side_effect=AssertionError('evaluator access')):
            for arm in FAMILIES:
                simulation(120001, name, arm, GRIDS[arm][3])
    print('PASS drift kernels: enriched schedule, boundary provenance, frozen parity, causality, privilege isolation', flush=True)
...
def synthetic():
    ...
    def adjust(r, arm):
        ...
        if arm in ('retain_drift', 'random_drift'):
            for f in r['memory'].values():
                for c in f.values():
                    if arm == 'random_drift':
...
def policy_checks():
    ...
    assert objective(tuning['window/0/121000']) == 1.
    assert all(i == 0 for i in manifest['screen']['indices'].values())
    assert manifest['screen']['selected']['oracle_nofallback'] == dict(FAST)
    assert manifest['screen']['selected']['retain_drift'] == manifest['screen']['selected']['random_drift'] == dict(FROZEN)
    assert manifest['screen']['random_frequency']['starts'] == 80
    ...
    for seed in SEEDS['confirmation']:
        for coord in bad[f'retain_drift/{seed}']['fixtures']['exactly_quiet']['coordinates'].values():
            coord['excess_mse'] = 0.
    ...
    bad = copy.deepcopy(tuning); bad['window/0/121000']['status'] = 'nonfinite'
    ...
    rejects(lambda: complete({k: r for k, r in tuning.items() if k != 'window/0/121000'}, 'tuning'))
    for control in ('window', 'sgd', 'adwin', 'random_drift', 'oracle_nofallback'):
    ...
    for key in ('adwin/123000', 'random_drift/123000', 'oracle_nofallback/123000', 'retain_drift/123000'):
        rejects(lambda k=key: performance_decision({a: r for a, r in rows.items() if a != k}))
    bad = copy.deepcopy(rows)
    for seed in SEEDS['confirmation']: bad[f'retain_drift/{seed}']['status'] = 'nonfinite'
    assert performance_decision(bad)['status'] == 'learning_negative'
    print('PASS drift policy: frozen menus, 75 vetoes, repaired both-perfect rule, strict ablation cells', flush=True)
...
def lifecycle_checks():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('study_v3_drift.require_committed', side_effect=ValueError('uncommitted')): rejects(lambda: confirm(directory))
        with patch('study_v3_drift.require_committed'):
        ...
        broken = copy.deepcopy(rows); broken['random_drift/123000']['manifest_digest'] = 'wrong'
        ...
        broken = copy.deepcopy(rows); broken['oracle_nofallback/123000']['privileged_timing'] = False
        envelope('confirmation', broken); rejects(lambda: read_stage(directory, 'confirmation'))
        broken = copy.deepcopy(rows); broken['retain_drift/123000']['memory']['core']['quiet']['resets'][0]['kind'] = 'random'
        rejects(lambda: absolute_summary(broken))
        broken = copy.deepcopy(rows); broken['retain_drift/123000']['memory']['core']['quiet']['regime_fast_share'] = 2.
        rejects(lambda: absolute_summary(broken))
        envelope('confirmation', rows)
        for k in tuning:
        ...
        with patch('study_v3_drift.require_committed'): rejects(lambda: confirm(directory))
    ...
    for arm in ARMS:
        config = GRIDS[arm][0] if arm in FAMILIES else dict(FAST) if arm == 'oracle_nofallback' else dict(FROZEN)
        row = measure_row(120002, arm, config, .001 if arm == 'random_drift' else None)
    ...
    print('PASS drift lifecycle: boundary provenance, source/manifest/stage gates, complete reports, all-arm development fixtures', flush=True)
...
def archive_checks(directory, reproduce):
    ...
    print('PASS drift archive tuning', len(tuning), flush=True)
    if not reproduce:
    ...
    for arm in FAMILIES:
        ...
        print('PASS drift reproduction tuning', arm, '96 rows', flush=True)
        ...
        for arm in ARMS:
            for seed in SEEDS['confirmation']:
                ...
                recreated[key] = measure_row(seed, arm, fresh_manifest['screen']['selected'][arm],
                    fresh_manifest['screen']['random_frequency']['probability'] if arm == 'random_drift' else None, fresh_manifest)
                compare(recreated[key], scientific(archived[key]), key, ignore=('manifest_digest',))
            print('PASS drift reproduction confirmation', arm, '32 rows', flush=True)
        compare(performance_decision(recreated), stored['decisions']['retain_drift'])
        compare(absolute_summary(recreated), stored['absolute'])
    print('PASS full drift reproduction; only reached partitions sampled', flush=True)
