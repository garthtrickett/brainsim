"""Horizon-regime equations, advancement authority and full reproduction."""
import argparse
import ast
import copy
import hashlib
...
from report_v3_slopetime import absolute_summary, publish_report, summarize
from study_v3_slopetime import (SOURCES, alarm_schedule, confirm, make_manifest, measure_row,
...
from v3_slopetime import ARMS, ESTIMATOR, FAMILIES, FAST, GRIDS, compose, random_uniforms, run
from v3_slopetime_policy import (KEYS, PROTOCOL, SEEDS, SPECS, TUNING_ALARM, complete, contrast,
...
SCHEDULED = [a for a in ARMS if a not in ('window', 'sgd', 'adwin')]
...
def kernel_checks():
    ...
    assert [c['Hs'] for c in GRIDS['slope_adwin']] == [16, 32, 64, 128, 256, 512, 1024, 2048, 3000, 4000, 5000, 6000]
    ...
    assert all(min(s) >= 140000 and max(s) < 146000 for s in parts)
    ...
    values = np.random.default_rng(140001).normal(size=600)
    ...
    # Horizon regime: fast wins inside 32, slope fills to Hs, pre-request convention.
    y = np.random.default_rng(140000).normal(size=(500, 2))
    alarms = np.zeros_like(y); alarms[[100, 400], 0] = 1.
    narrow = compose(y, alarms, 16)
    assert (narrow[7][:, 0] != 2).all()
    wide = compose(y, alarms, 6000)
    assert (wide[7][101:132, 0] == 1).all() and (wide[7][133:400, 0] == 2).all() and (wide[7][401:432, 0] == 1).all()
    assert wide[7][100, 0] != 2 and wide[7][400, 0] != 1
    assert (wide[7][:, 1] == 0).all()
    slopes = compose(y, alarms, 128)
    assert (slopes[7][101:132, 0] == 1).all() and (slopes[7][133:228, 0] == 2).all()
    assert (slopes[7][229:400, 0] == 0).all() and (slopes[7][401:432, 0] == 1).all()
    # Frozen parity: estimator, dual base and delegations.
    for a, b in zip(compose(y, alarms, 128)[0], run(y, 'slope_adwin', {'Hs': 128}, schedule=alarms)[0]):
        np.testing.assert_array_equal(a, b)
    triggers = np.zeros_like(y); triggers[200] = 1.
    for a, b in zip(run(y, 'reference', dict(ESTIMATOR), schedule=triggers),
                     dual_run(y, triggers, .1, 32)):
    ...
    uniforms = random_uniforms(140000, 0, len(y), 2)
    for arm in ARMS:
        config = GRIDS[arm][3] if arm in FAMILIES else dict(ESTIMATOR) if arm == 'reference' else (
            dict(FAST) if arm == 'oracle_nofallback' else dict(GRIDS['slope_adwin'][3]))
        if arm in ('window', 'sgd', 'adwin'):
            kwargs = {}
        elif arm == 'random_slope':
            kwargs = {'uniforms': uniforms, 'probability': .01}
        else:
            kwargs = {'schedule': adwin_run(y, .1, 1)[1] if arm in ('slope_adwin', 'oracle_nofallback') else triggers}
    ...
    np.testing.assert_array_equal(uniforms[:200], random_uniforms(140000, 0, 200, 2))
    np.testing.assert_array_equal(uniforms[:, :1], random_uniforms(140000, 0, len(y), 1))
    assert not np.array_equal(uniforms, random_uniforms(140000, 1, len(y), 2))
    assert not np.array_equal(uniforms[:, 0], uniforms[:, 1])
    rejects(lambda: run(y, 'slope_adwin', {'Hs': 0}, schedule=alarms))
    rejects(lambda: run(y, 'slope_adwin', {'Hs': 32.5}, schedule=alarms))
    rejects(lambda: run(y, 'reference', {'method': 'ols', 'W': 64}, schedule=triggers))
    rejects(lambda: run(np.full_like(y, np.nan), 'window', {'window': 16}))
    rejects(lambda: run(y, 'slope_adwin', {'Hs': 128}))
    for name in FIXTURES:
        data = fixture(140001, name)
        assert np.array_equal(alarm_schedule(data, .1, 1), adwin_run(data.y, .1, 1)[1])
        with patch('study_v3_slopetime.alarm_schedule', side_effect=AssertionError('evaluator access')):
            for arm in ('window', 'sgd', 'adwin'):
                simulation(140001, name, arm, GRIDS[arm][3])
    print('PASS slopetime kernels: horizon regimes, frozen parity, alarm provenance, causality, privilege isolation', flush=True)
...
def synthetic():
    ...
    for name, coordinates in coords.items():
        ...
        for c in coordinates:
            ...
            row['memory'][name][c] = {'kind': 'window', 'reset_count': 2,
                'resets': [{'index': t, 'width_before': 32, 'width_after': 1, 'discarded': 32, 'kind': 'observed'} for t in (2000, 4000)],
                'width_mean': 25., 'width_min': 1, 'width_max': 32, 'width_final': 32, 'total_discarded': 5968,
                'events': copy.deepcopy(events), 'provenance': 'none'}
    def adjust(r, arm):
        prov = {'slope_adwin': 'observed', 'oracle_nofallback': 'observed', 'reference': 'true', 'random_slope': 'random'}.get(arm, 'none')
        if arm == 'sgd':
        ...
        elif arm in ('slope_adwin', 'random_slope'):
            for f in r['memory'].values():
                for c in f.values():
                    if arm == 'random_slope':
    ...
    for a in ARMS:
        for seed in SEEDS['confirmation']:
            ...
            if a in ('slope_adwin', 'reference', 'random_slope', 'oracle_nofallback'):
...
def policy_checks():
    ...
    assert objective(tuning['slope_adwin/0/141000']) == 1.
    assert all(i == 0 for i in manifest['screen']['indices'].values())
    assert manifest['screen']['selected']['reference'] == dict(ESTIMATOR)
    assert manifest['screen']['selected']['random_slope'] == manifest['screen']['selected']['slope_adwin']
    ...
    assert performance_decision(rows, 'slope_adwin')['status'] == 'learning_positive'
    assert len(performance_decision(rows, 'slope_adwin')['cells']) == 45
    for arm in ('reference', 'random_slope', 'oracle_nofallback'):
        assert performance_decision(rows, arm)['status'] == 'learning_positive'
    bad = copy.deepcopy(tuning); bad['slope_adwin/0/141000']['status'] = 'nonfinite'
    assert select(bad)['indices']['slope_adwin'] == 1
    for k in bad:
        if k.startswith('slope_adwin/'): bad[k]['status'] = 'nonfinite'
    assert select(bad)['status'] == 'tuning_inconclusive'
    rejects(lambda: complete({k: r for k, r in tuning.items() if k != 'slope_adwin/0/141000'}, 'tuning'))
    for control in ('window', 'sgd', 'adwin'):
        for label, fields in SPECS:
            ...
            result = performance_decision(bad, 'slope_adwin')
    ...
    for key in ('adwin/143000', 'slope_adwin/143000', 'reference/143000'):
        rejects(lambda k=key: performance_decision({a: r for a, r in rows.items() if a != k}, 'slope_adwin'))
    bad = copy.deepcopy(rows)
    for seed in SEEDS['confirmation']: bad[f'slope_adwin/{seed}']['status'] = 'nonfinite'
    assert performance_decision(bad, 'slope_adwin')['status'] == 'learning_negative'
    print('PASS slopetime policy: frozen menus, 45-cell gate, diagnostic instruments', flush=True)
...
def lifecycle_checks():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch('study_v3_slopetime.require_committed', side_effect=ValueError('uncommitted')): rejects(lambda: confirm(directory))
        with patch('study_v3_slopetime.require_committed'):
            assert verified_manifest(directory) == manifest
            changed = copy.deepcopy(manifest); changed['screen']['indices']['slope_adwin'] = 5; write(directory/'manifest.json', changed)
        ...
        broken = copy.deepcopy(rows); broken['reference/143000']['manifest_digest'] = 'wrong'
        ...
        broken = copy.deepcopy(rows); broken['slope_adwin/143000']['privileged_timing'] = False
        envelope('confirmation', broken); rejects(lambda: read_stage(directory, 'confirmation'))
        broken = copy.deepcopy(rows); broken['slope_adwin/143000']['memory']['core']['quiet']['provenance'] = 'true'
        rejects(lambda: absolute_summary(broken))
        broken = copy.deepcopy(rows); broken['slope_adwin/143000']['memory']['core']['quiet']['regime_slope_share'] = 2.
        rejects(lambda: absolute_summary(broken))
        envelope('confirmation', rows)
        for k in tuning:
            if k.startswith('slope_adwin/'): tuning[k]['status'] = 'nonfinite'
        ...
        with patch('study_v3_slopetime.require_committed'): rejects(lambda: confirm(directory))
    ...
    for arm in ARMS:
        config = GRIDS[arm][0] if arm in FAMILIES else dict(ESTIMATOR) if arm == 'reference' else (
            dict(FAST) if arm == 'oracle_nofallback' else dict(GRIDS['slope_adwin'][0]))
        row = measure_row(140002, arm, config, alarm if arm in ('slope_adwin', 'oracle_nofallback') else None,
                          .001 if arm == 'random_slope' else None)
        assert row['status'] == 'ok' and len(row['fixtures']) == 5
        assert row['privileged_timing'] == (arm not in ('window', 'sgd', 'adwin'))
    print('PASS slopetime lifecycle: provenance accounting, source/manifest/stage gates, complete reports, all-arm development fixtures', flush=True)
...
def archive_checks(directory, reproduce):
    ...
    if not reproduce:
        return
    print('PASS slopetime archive tuning', len(tuning), flush=True)
    fresh = {}
    for arm in FAMILIES:
        for i, config in enumerate(GRIDS[arm]):
            for seed in SEEDS['tuning']:
                key = f'{arm}/{i}/{seed}'
                alarm = TUNING_ALARM if arm == 'slope_adwin' else None
                fresh[key] = measure_row(seed, arm, config, alarm)
                compare(fresh[key], scientific(tuning[key]), key)
        print('PASS slopetime reproduction tuning', arm, '96 rows', flush=True)
        ...
        for arm in ARMS:
            for seed in SEEDS['confirmation']:
                ...
                recreated[key] = measure_row(seed, arm, fresh_manifest['screen']['selected'][arm],
                    alarm if arm in ('slope_adwin', 'oracle_nofallback') else None,
                    fresh_manifest['screen']['random_frequency']['probability'] if arm == 'random_slope' else None,
                    fresh_manifest)
                compare(recreated[key], scientific(archived[key]), key, ignore=('manifest_digest',))
            print('PASS slopetime reproduction confirmation', arm, '32 rows', flush=True)
        for arm in ('slope_adwin', 'reference', 'random_slope', 'oracle_nofallback'):
            compare(performance_decision(recreated, arm), stored['decisions'][arm])
        compare(absolute_summary(recreated), stored['absolute'])
    print('PASS full slopetime reproduction; only reached partitions sampled', flush=True)
