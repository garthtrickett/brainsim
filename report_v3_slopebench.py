"""Slope-benchmark results with new-ramp accounting."""
import argparse
import json
...
from study_v3_slopebench import DIRECTORY, make_manifest, read_stage
from v3_slopebench_policy import ARMS, GRIDS, KEYS, SEEDS, digest, interval, metric_values, performance_decision
...
def markdown(summary):
    lines = ['# V3 slope broader benchmark', '', f'Disposition: **{summary["status"]}**.', '',
             '| Stage | Rows |', '| --- | ---: |']
    ...
    lines += ['', '## Finite-menu tuning', '', 'Each searched family: 12 configurations ×8 seeds ×5 fixtures. '
              ...
              'The candidate configuration is frozen from the slope manifest.', '',
    ...
    lines += ['', 'Endpoint choices remain in the registered finite menu; no extension or global-optimum claim. '
              'Oracle_nofallback is the fixed fast base on the enriched schedule. Random_slope uses the frozen estimator.']
    ...
    if summary['absolute']:
        ...
        lines += ['', 'Width counts retained observations, not physical storage. SGD uses exponential weights '
                  ...
                  'provenance in every record and never mix. Ramp MSE averages pre-update squared error over '
                  'the ramp segment; it is None-free on ramp fixtures and absent elsewhere.', '']
    ...
    lines += ['', 'Configuration budgets are equal, not CPU costs. Times exclude archive writes and share '
              ...
              'Intervals resample whole seeds10000 times with seed155000. Finite menus and synthetic fixtures '
              ...
              '`python check_v3_slopebench.py --evidence results/v3-slopebench --reproduce` regenerates every '
              'reached row and scientific manifest/summary at rtol1e-11/atol1e-13. '
              '`python report_v3_slopebench.py --check` verifies both generated reports.', '']
    return '\n'.join(lines)
