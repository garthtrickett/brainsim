"""Drift-timing tolerance curve with per-level instruments."""
import argparse
import json
...
from study_v3_drifttol import DIRECTORY, make_manifest, read_stage
from v3_drifttol import ARMS, GRIDS
from v3_drifttol_policy import (KEYS, LEVELS, SEEDS, digest, interval, metric_values,
                                performance_decision, tolerance_point)
from v3_persistent import FIXTURES

MEASURED = ['reference'] + LEVELS + ['drop_50', 'oracle_nofallback']
            ...
            for coord in values[0]['coordinates']:
                ...
                for record in memory:
                    ...
                    if arm == 'sgd':
                        ...
                        if record['kind'] != ('dual' if arm in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50') else 'window'):
                            raise ValueError('invalid memory kind')
                        check_widths(record)
                        for reset in record['resets']:
                            ...
                            if arm in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50', 'oracle_nofallback'):
                                if reset.get('kind') not in ('target', 'drift'):
                ...
                dual = arm in ('reference', 'jitter_32', 'jitter_256', 'jitter_1024', 'drop_50')
...
def summarize(directory):
    ...
    if (directory/'confirmation.json').exists():
        ...
        result['decisions'] = {a: performance_decision(rows, a) for a in MEASURED}
        result['tolerance'] = tolerance_point(rows)
        result['status'], result['closed'] = result['decisions']['reference']['status'], True
...
def markdown(summary):
    lines = ['# V3 drift-timing tolerance', '', f'Disposition: reference **{summary["status"]}**; tolerance **{summary["tolerance"]}**.', '',
             '| Stage | Rows |', '| --- | ---: |']
    ...
    lines += ['', '## Finite-menu tuning', '', 'Each searched family: 12 configurations ×8 seeds ×5 fixtures. '
              ...
              'The slope configuration is frozen from the slope manifest; perturbed arms run in confirmation only.', '',
    ...
    if summary['tolerance'] is not None:
        lines += ['', '## Tolerance measurement (diagnostic arms)', '',
    ...
    for arm, decision in summary['decisions'].items():
        if arm != 'reference':
            continue
        lines += ['', f'## {arm}: {decision["status"]} (unperturbed baseline, diagnostic)', '',
    ...
    if summary['absolute']:
        ...
        lines += ['', 'Width counts retained observations, not physical storage. SGD uses exponential weights '
                  'and has no literal window. Granted, perturbed and oracle schedules are distinguished by '
                  'provenance in every record and never mix.', '']
    ...
    lines += ['', 'Configuration budgets are equal, not CPU costs. Times exclude archive writes and share '
              ...
              'Perturbed schedules are diagnostic: jittered/dropped times are evaluator-built from true '
              'boundaries, not observable-derived. The tolerance curve authorizes no follow-up; the reading rules '
              'in the registration decide what follows.', '',
              'Intervals resample whole seeds10000 times with seed175000. Finite menus and synthetic fixtures '
              ...
              '`python check_v3_drifttol.py --evidence results/v3-drifttol --reproduce` regenerates every '
              'reached row and scientific manifest/summary at rtol1e-11/atol1e-13. '
              '`python report_v3_drifttol.py --check` verifies both generated reports.', '']
    return '\n'.join(lines)
